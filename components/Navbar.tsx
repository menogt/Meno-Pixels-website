    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-7">
          {links.map((l) => (
            <a key={l.label} href={l.href}
              ...
              {l.label}
              <span className="absolute -bottom-0.5 left-0 w-0 h-px bg-red-400 transition-all duration-300 group-hover:w-full" />
            </a>
          ))}
        </nav>
        ...
        <a href="https://wa.me/94783780057" target="_blank" rel="noopener noreferrer"
          ...
          style={{ background: "linear-gradient(135deg, #FF3B30, #DC2626)", boxShadow: "0 4px 20px rgba(255,59,48,0.35)" }}>
          Get Free Audit
        </a>
            ...
            <div className="px-6 py-4 flex flex-col gap-4">
              {links.map((l) => (
                <a key={l.label} href={l.href} onClick={() => setOpen(false)}
                  className="text-sm text-slate-300 hover:text-white py-1">{l.label}</a>
              ))}
              <a href="https://wa.me/94783780057" target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center px-4 py-2.5 rounded-lg text-sm font-semibold text-white mt-2"
