  ...
  return (
    <div className="glass gradient-border rounded-2xl p-4 w-52 shadow-2xl">
      ...
      <div className="mt-3 flex gap-1.5">
        <div className="h-8 flex-1 rounded-lg" style={{ background: "rgba(139,92,246,0.25)", border: "1px solid rgba(139,92,246,0.4)" }} />
        <div className="h-8 w-8 rounded-lg" style={{ background: "rgba(6,182,212,0.15)", border: "1px solid rgba(6,182,212,0.3)" }} />
      </div>
    </div>
  );
}
...
function EcomCard() {
  return (
    <div className="glass gradient-border rounded-2xl p-4 w-48 shadow-2xl">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-6 h-6 rounded-md flex items-center justify-center" style={{ background: "linear-gradient(135deg,#EC4899,#8B5CF6)" }}>
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {[1,2,3,4].map(i => (
          <div key={i} className="h-12 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }} />
        ))}
      </div>
      <div className="mt-2 h-7 rounded-lg w-full" style={{ background: "linear-gradient(135deg,rgba(139,92,246,0.4),rgba(236,72,153,0.3))" }} />
    </div>
  );
}
      ...
      <div className="grid grid-cols-3 gap-1 mb-2">
        {["M","T","W","T","F","S","S"].map((d,i) => (
          <div key={i} className={`h-6 rounded-md flex items-center justify-center text-[9px] font-semibold ${i===2?"text-white":"text-slate-500"}`}
            style={{ background: i===2 ? "rgba(139,92,246,0.5)" : "rgba(255,255,255,0.04)" }}>
            {d}
          </div>
        ))}
      </div>
  ...
  return (
    <section ref={ref} id="home" className="relative min-h-screen flex items-center overflow-hidden pt-16">
      ...
      <div className="absolute inset-0">
        <motion.div
          style={{ y: bgY, background: "radial-gradient(circle, rgba(139,92,246,0.12) 0%, rgba(236,72,153,0.06) 50%, transparent 70%)" }}
        ...
        <div className="absolute bottom-0 left-0 w-72 h-72 rounded-full opacity-15"
          style={{ background: "radial-gradient(circle, rgba(236,72,153,0.3), transparent 70%)", filter: "blur(60px)" }} />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 w-full">
        <div className="grid lg:grid-cols-2 gap-16 items-center min-h-screen py-24 lg:py-0">
          ...
          <motion.div style={{ y: textY }}>
            ...
            <motion.h1
              initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.25, duration:0.8, ease:[0.22,1,0.36,1] }}
            ...
            >
              <a href="https://wa.me/947XXXXXXXX" target="_blank" rel="noopener noreferrer"
                ...
                style={{ background: "linear-gradient(135deg,#8B5CF6,#EC4899)", boxShadow: "0 8px 30px rgba(139,92,246,0.35)" }}>
              ...
              <div className="flex items-center gap-2">
                <div className="flex -space-x-1.5">
                  {["#8B5CF6","#EC4899","#06B6D4"].map((c,i) => (
                    <div key={i} className="w-7 h-7 rounded-full border-2 border-[#030305]" style={{ background: c }} />
                  ))}
                </div>
          ...
          <div className="hidden lg:block relative h-[600px]">
            ...
            <motion.div
              style={{ y: card3Y }}
              className="absolute top-4 right-56 animate-float-b z-10"
            >
              <GoogleCard />
            </motion.div>
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 rounded-full animate-pulse-glow"
              style={{ background: "radial-gradient(circle, rgba(139,92,246,0.18), transparent 70%)", filter: "blur(30px)" }} />
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="w-px h-10 overflow-hidden">
          <motion.div className="w-full h-full bg-gradient-to-b from-transparent via-purple-500 to-transparent"
